...
void main() {
	...
	vec3 texSample = texture(inputImg, leftUpCoord).rgb;
	float _min  = texSample.r;
	float _mean = texSample.g;
	float _max  = texSample.b;

	vec2 currCoord;
	for (int j = 1; j < 3; ++j) {
		currCoord = leftUpCoord + vec2(inputTexPixelSize.x*j, 0);
		texSample = texture(inputImg, currCoord).rgb;
		_min = min(_min, texSample.r);
		_mean += texSample.g;
		_max = max(_max, texSample.b);
	}
	for (int i = 1; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
